...
function getToken(sid: string): string | undefined {
...
export async function POST(req: NextRequest) {
  ...
  if (listRes.ok) {
    ...
    if (hooks.find((h) => h.config.url === `${appUrl}/api/webhook`)) {
      addConnectedRepo(repoFullName, sid);
...
export async function GET(req: NextRequest) {
  const sid = getSessionId(req);
  return NextResponse.json({ repos: sid ? getConnectedRepos(sid) : [] });
}
...
export async function DELETE(req: NextRequest) {
  ...
  removeConnectedRepo(repoFullName, sid);
  return NextResponse.json({ ok: true });
}
